...
// JWEBKIT FIX: 
  ...
  scope.__jwk_fix___elementDeclarations = [];
  var polymerStub = function(name, dictionary) {
    if (typeof name !== "string" && arguments.length === 1) {
      Array.prototype.push.call(arguments, document._currentScript);
    }
    scope.__jwk_fix___elementDeclarations.push(arguments);
  };
  ...
  scope.consumeDeclarations = function(callback) {
    ...
    if (callback) {
      callback(scope.__jwk_fix___elementDeclarations);
    }
    scope.__jwk_fix___elementDeclarations = null;
  };
